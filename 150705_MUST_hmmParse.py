...
# script to get the annotation from HMM search for each gene, for KEGG, metaCyc, Swiss-Prot or TIGR Pfam;
# this script does not work for the Pfam annotations, because they can contain underscores which do not signify just annother HMM for the same function
# gives for every gene the node, together with the score of the best hit and the number of hits
# takes 5 inputs: the hit files for each database (output of consolidate_hmmscan_results.pl);
