...
#!!!this version handles the mix up of start- and stop-codons in the prodigal .tab output and but returns a .tab with mixed-up start- and stop-codons, like prodigal!!
...
foreach my $k (sort keys %{$annot}) {
	...
	if ($$feat{'variants'}) {
		...
		foreach my $nucv (@nucvs) {
		    ...
		    my $startc = $$feat{'startcodon'};
		    my $compl = $$feat{'completeness'};
		    if ($pro eq $prov) {
			++$syn;
		    } else {
			++$non;
			if (($$feat{'strand'} eq "+" && $$feat{'startcodon'} eq "yes" && substr($prov,0,3) ne substr($pro,0,3)) || ($$feat{'strand'} eq "-" && $$feat{'stopcodon'} eq "yes" && substr($prov,0,3) ne substr($pro,0,3))){
			...
			} elsif (substr($pro,-1) ne "*" && substr($prov,-1) eq "*"){
			    if ($$feat{'strand'} eq "+") {
				$stopc = "yes";
			...
			print $o_prov qq(\>$idv\n$prov\n) if ($print);
			print $o_tabv qq(@{[join("\t", $idv,$$feat{'strand'},$$feat{'start'},$$feat{'stop'},"na",$startc,$stopc,$compl)]}\n) if ($print);
		    }
		    $vind++;
		}
		$vcnt++;
